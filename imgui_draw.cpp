...
#pragma GCC diagnostic ignored "-Wtype-limits"  // warning: comparison is always
                                                // true due to limited range of
...
void ImDrawList::ClearFreeMemory() {
  ...
  for (int i = 0; i < _Channels.Size; i++) {
    if (i == 0)
      memset(&_Channels[0], 0, sizeof(_Channels[0]));  // channel 0 is a copy of
                                                       // CmdBuffer/IdxBuffer,
...
// Use macros because C++ is a terrible language, we want guaranteed inline, no
// code in header, and no overhead in Debug mode
...
void ImDrawList::PushClipRectFullScreen() {
  PushClipRect(ImVec2(GNullClipRect.x, GNullClipRect.y),
               ImVec2(GNullClipRect.z, GNullClipRect.w));
  // PushClipRect(GetVisibleRect());   // FIXME-OPT: This would be more correct
  // but we're not supposed to access ImGuiContext from here?
}
...
void ImDrawList::ChannelsSplit(int channels_count) {
  ...
  // _Channels[] (24 bytes each) hold storage that we'll swap with
...
void ImDrawList::AddPolyline(const ImVec2* points, const int points_count,
  ...
  if (anti_aliased) {
    ...
    // Temporary buffer
    ImVec2* temp_normals =
        (ImVec2*)alloca(points_count * (thick_line ? 5 : 3) * sizeof(ImVec2));
...
static void PathBezierToCasteljau(ImVector<ImVec2>* path, float x1, float y1,
                                  ...
                                  float x4, float y4, float tess_tol,
                                  int level) {
  float dx = x4 - x1;
  float dy = y4 - y1;
...
void ImDrawList::PathBezierCurveTo(const ImVec2& p2, const ImVec2& p3,
  ...
  if (num_segments == 0) {
    ...
    PathBezierToCasteljau(&_Path, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y, p4.x,
                          p4.y, GImGui->Style.CurveTessellationTol, 0);
  } else {
...
void ImDrawList::PathRect(const ImVec2& a, const ImVec2& b, float rounding,
  ...
  rounding = ImMin(
      rounding,
      fabsf(b.x - a.x) *
              (((rounding_corners & corners_top) == corners_top) ||
                       ((rounding_corners & corners_bottom) == corners_bottom)
                   ? 0.5f
                   : 1.0f) -
          1.0f);
  rounding = ImMin(
      rounding,
      fabsf(b.y - a.y) *
              (((rounding_corners & corners_left) == corners_left) ||
                       ((rounding_corners & corners_right) == corners_right)
                   ? 0.5f
                   : 1.0f) -
          1.0f);
  ...
  if (rounding <= 0.0f || rounding_corners == 0) {
    ...
    const float rounding_tl =
        (rounding_corners & ImGuiCorner_TopLeft) ? rounding : 0.0f;
    const float rounding_tr =
        (rounding_corners & ImGuiCorner_TopRight) ? rounding : 0.0f;
    const float rounding_br =
        (rounding_corners & ImGuiCorner_BotRight) ? rounding : 0.0f;
    const float rounding_bl =
        (rounding_corners & ImGuiCorner_BotLeft) ? rounding : 0.0f;
...
void ImDrawList::AddText(const ImFont* font, float font_size, const ImVec2& pos,
  ...
  // IMPORTANT: This is one of the few instance of breaking the encapsulation of
  // ImDrawList, as we pull this from ImGui state, but it is just SO useful.
  // Might just move Font/FontSize to ImDrawList?
  if (font == NULL) font = GImGui->Font;
  if (font_size == 0.0f) font_size = GImGui->FontSize;

  IM_ASSERT(font->ContainerAtlas->TexID ==
            _TextureIdStack.back());  // Use high-level ImGui::PushFont() or
                                      // low-level ImDrawList::PushTextureId()
                                      // to change font.
...
const char FONT_ATLAS_DEFAULT_TEX_DATA_PIXELS
...
void ImFontAtlas::GetTexDataAsRGBA32(unsigned char** out_pixels, int* out_width,
  ...
  if (!TexPixelsRGBA32) {
    ...
    TexPixelsRGBA32 =
        (unsigned int*)ImGui::MemAlloc((size_t)(TexWidth * TexHeight * 4));
...
void ImFontAtlas::CalcCustomRectUV(const CustomRect* rect, ImVec2* out_uv_min,
                                   ImVec2* out_uv_max) {
  IM_ASSERT(TexWidth > 0 && TexHeight > 0);  // Font atlas needs to be built
                                             // before we can calculate UV
                                             // coordinates
...
bool ImFontAtlas::Build() { return ImFontAtlasBuildWithStbTruetype(this); }
...
bool ImFontAtlasBuildWithStbTruetype(ImFontAtlas* atlas) {
  ...
  // they wish. Width doesn't really matter much, but some API/GPU have texture
  ...
  memset(buf_rects, 0,
         total_glyphs_count * sizeof(stbrp_rect));  // Unnecessary but let's
                                                    // clear this for the sake
                                                    // of sanity.
  ...
  for (int input_i = 0; input_i < atlas->ConfigData.Size; input_i++) {
    ...
    if (cfg.RasterizerMultiply != 1.0f) {
      ...
      for (const stbrp_rect* r = tmp.Rects; r != tmp.Rects + tmp.RectsCount;
           r++)
        if (r->was_packed)
          ImFontAtlasBuildMultiplyRectAlpha8(multiply_table, spc.pixels, r->x,
                                             r->y, r->w, r->h,
                                             spc.stride_in_bytes);
    }
    tmp.Rects = NULL;
  ...
  for (int input_i = 0; input_i < atlas->ConfigData.Size; input_i++) {
    ImFontConfig& cfg = atlas->ConfigData[input_i];
    ImFontTempBuildData& tmp = tmp_array[input_i];
    ImFont* dst_font = cfg.DstFont;  // We can have multiple input fonts writing
                                     // into a same destination font (when using
                                     // MergeMode=true)

    const float font_scale =
        stbtt_ScaleForPixelHeight(&tmp.FontInfo, cfg.SizePixels);
    int unscaled_ascent, unscaled_descent, unscaled_line_gap;
    stbtt_GetFontVMetrics(&tmp.FontInfo, &unscaled_ascent, &unscaled_descent,
                          &unscaled_line_gap);
    ...
    dst_font->FallbackGlyph = NULL;  // Always clear fallback so FindGlyph can
                                     // return NULL. It will be set again in
                                     // BuildLookupTable()
    for (int i = 0; i < tmp.RangesCount; i++) {
      ...
      for (int char_idx = 0; char_idx < range.num_chars; char_idx += 1) {
        ...
        stbtt_GetPackedQuad(range.chardata_for_range, atlas->TexWidth,
                            atlas->TexHeight, char_idx, &dummy_x, &dummy_y, &q,
                            0);
        dst_font->AddGlyph((ImWchar)codepoint, q.x0 + off_x, q.y0 + off_y,
                           q.x1 + off_x, q.y1 + off_y, q.s0, q.t0, q.s1, q.t1,
                           pc.xadvance);
      }
    }
  }
...
void ImFontAtlasBuildRegisterDefaultCustomRects(ImFontAtlas* atlas) {
  if (atlas->CustomRectIds[0] < 0)
    atlas->CustomRectIds[0] =
        atlas->AddCustomRectRegular(FONT_ATLAS_DEFAULT_TEX_DATA_ID,
...
void ImFontAtlasBuildPackCustomRects(ImFontAtlas* atlas,
  ...
  IM_ASSERT(user_rects.Size >= 1);  // We expect at least the default custom
                                    // rects to be registered, else something
                                    // went wrong.
...
const ImWchar* ImFontAtlas::GetGlyphRangesJapanese() {
  // Store the 1946 ideograms code points as successive offsets from the initial
  // unicode codepoint 0x4E00. Each offset has an implicit +1. This encoding is
  // designed to helps us reduce the source code size.
  // FIXME: Source a list of the revised 2136 joyo kanji list from 2010 and
  // rebuild this. The current list was sourced from
  // http://theinstructionlimit.com/author/renaudbedardrenaudbedard/page/3 Note
  // that you may use ImFontAtlas::GlyphRangesBuilder to create your own ranges,
  // by merging existing ranges or adding new characters.
...
void ImFont::AddRemapChar(ImWchar dst, ImWchar src, bool overwrite_dst) {
  IM_ASSERT(IndexLookup.Size > 0);  // Currently this can only be called AFTER
                                    // the font has been built, aka after
                                    // calling ImFontAtlas::GetTexDataAs*()
                                    // function.
...
const ImFontGlyph* ImFont::FindGlyph(unsigned short c) const {
  if (c < IndexLookup.Size) {
    ...
    if (i != (unsigned short)-1) return &Glyphs.Data[i];
  }
  ...
  while (s < text_end) {
    if (word_wrap_enabled) {
      ...
      if (!word_wrap_eol) {
        ...
        if (word_wrap_eol == s)  // Wrap_width is too small to fit anything.
                                 // Force displaying 1 character to minimize the
                                 // height discontinuity.
          word_wrap_eol++;  // +1 may not be a character start point in UTF-8
                            // but it's ok because we use s >= word_wrap_eol
                            // below
      }
...
void ImFont::RenderText(ImDrawList* draw_list, float size, ImVec2 pos,
  ...
  if (!text_end)
    text_end = text_begin + strlen(text_begin);  // ImGui functions generally
                                                 // already provides a valid
                                                 // text_end, so this is merely
                                                 // to handle direct calls.
  ...
  while (s < text_end) {
    if (word_wrap_enabled) {
      ...
      if (!word_wrap_eol) {
        word_wrap_eol =
            CalcWordWrapPositionA(scale, s, text_end, wrap_width - (x - pos.x));
        if (word_wrap_eol == s)  // Wrap_width is too small to fit anything.
                                 // Force displaying 1 character to minimize the
                                 // height discontinuity.
          word_wrap_eol++;  // +1 may not be a character start point in UTF-8
    ...
    if (const ImFontGlyph* glyph = FindGlyph((unsigned short)c)) {
      ...
      // Arbitrarily assume that both space and tabs are empty glyphs as an
      // optimization
      if (c != ' ' && c != '\t') {
