...
// ImGui::ShowTestWindow(). During development, you can call
// ImGui::ShowTestWindow() in your code to learn about various features of
...
// IMGUI_DISABLE_TEST_WINDOWS in imconfig.h for the same effect. If you want to
// link core ImGui in your public builds but not those test windows, #define
// IMGUI_DISABLE_TEST_WINDOWS in imconfig.h and those functions will be empty.
// For any other case, if you have ImGui available you probably want this to be
// available for reference and execution. Thank you, -Your beloved friend,
// imgui_demo.cpp (that you won't delete)

// Message to beginner C/C++ programmer about the meaning of 'static': in this
// demo code, we frequently we use 'static' variables inside functions. We do
// this as a way to gather code and data in the same place, make the demo code
// faster to read, faster to write, and smaller. A static variable persist
// across calls, so it is essentially like a global variable but declared inside
// the scope of the function. It also happens to be a convenient way of storing
// simple UI related information as long as your function doesn't need to be
// reentrant or used in threads. This may be a pattern you want to use in your
// code (simple is beautiful!), but most of the real data you would be editing
// is likely to be stored outside your function.
...
#pragma clang diagnostic ignored \
    "-Wint-to-void-pointer-cast"  // warning : cast to 'void *' from smaller
                                  // integer type 'int'
#pragma clang diagnostic ignored "-Wformat-security"  // warning : warning:
...
#pragma GCC diagnostic ignored "-Wint-to-pointer-cast"  // warning: cast to
                                                        ...
                                                        // of different size
#pragma GCC diagnostic ignored "-Wformat-security"  // warning : format string
...
void ImGui::ShowTestWindow(bool* p_open) {
  ...
  if (ImGui::BeginMenuBar()) {
    ...
    if (ImGui::BeginMenu("Examples")) {
      ...
      ImGui::MenuItem("Manipulating window title", NULL,
  ...
  if (ImGui::CollapsingHeader("Help")) {
    ImGui::TextWrapped(
        "This window is being created by the ShowTestWindow() function. Please "
        "refer to the code for programming reference.\n\nUser Guide:");
    ImGui::ShowUserGuide();
  }
  ...
  if (ImGui::CollapsingHeader("Window options")) {
    ...
    if (ImGui::TreeNode("Logging")) {
      ImGui::TextWrapped(
          ...
          "the content of a window or a block. Tree nodes can be automatically "
          "expanded. You can also call ImGui::LogText() to output directly to "
          "the log without a visual output.");
      ImGui::LogButtons();
      ImGui::TreePop();
    }
  }
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    if (ImGui::TreeNode("Basic")) {
      ...
      ImGui::Combo("combo", &item,
                   "aaaa\0bbbb\0cccc\0dddd\0eeee\0\0");  // Combo using values
                                                         // packed in a single
                                                         // constant string (for
                                                         // really quick combo)
      ...
      static int item2 = -1;
      ImGui::Combo("combo scroll", &item2, items,
                   IM_ARRAYSIZE(items));  // Combo using proper array. You can
                                          // also pass a callback to retrieve
                                          // array value, no need to create/copy
                                          // an array just for that.
...

      {
        ...
        ImGui::SliderFloat("slider log float", &f2, -10.0f, 10.0f, "%.4f",
                           3.0f);
      ...
      ShowHelpMarker(
          ...
          "the colored square to show options.\nCTRL+click on individual "
      ...
      // IM_ARRAYSIZE(listbox_items), 4);  ImGui::PopItemWidth();
      ...
      if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
        ...
        static int selection_mask =
            (1 << 2);  // Dumb representation of what may be user-side selection
                       // state. You may carry selection state inside or outside
                       // your objects in whatever format you see fit.
        int node_clicked = -1;  // Temporary storage of what node we have
                                // clicked to process selection at the end of
                                // the loop. May be a pointer to your own node
                                // type, etc.
        ImGui::PushStyleVar(ImGuiStyleVar_IndentSpacing,
                            ImGui::GetFontSize() * 3);  // Increase spacing to
                                                        // differentiate leaves
                                                        // from expanded
                                                        // contents.
        for (int i = 0; i < 6; i++) {
          ...
          if (i < 3) {
            ...
            // Leaf: The only reason we have a TreeNode at all is to allow
            // selection of the leaf. Otherwise we can use BulletText() or
        ...
        if (node_clicked != -1) {
          ...
          if (ImGui::GetIO().KeyCtrl)
            selection_mask ^= (1 << node_clicked);  // CTRL+click to toggle
                ...
                // selection behavior you want, this commented bit preserve
    ...
    if (ImGui::TreeNode("Text")) {
      ...
      if (ImGui::TreeNode("Word Wrapping")) {
        // Using shortcut. You can use PushTextWrapPos()/PopTextWrapPos() for
        // more flexibility.
        ImGui::TextWrapped(
      ...
      if (ImGui::TreeNode("UTF-8 Text")) {
        ...
        // signature')
        // - HOWEVER, FOR THIS DEMO FILE, BECAUSE WE WANT TO SUPPORT COMPILER,
        // WE ARE *NOT* INCLUDING RAW UTF-8 CHARACTERS IN THIS SOURCE FILE.
        //   Instead we are encoding a few string with hexadecimal constants.
    ...
    if (ImGui::TreeNode("Images")) {
      ...
      ImGui::Image(tex_id, ImVec2(tex_w, tex_h), ImVec2(0, 0), ImVec2(1, 1),
      ...
      if (ImGui::IsItemHovered()) {
        ...
        ImGui::Text("Max: (%.2f, %.2f)", focus_x + focus_sz,
                    focus_y + focus_sz);
        ImVec2 uv0 = ImVec2((focus_x) / tex_w, (focus_y) / tex_h);
        ImVec2 uv1 =
            ImVec2((focus_x + focus_sz) / tex_w, (focus_y + focus_sz) / tex_h);
        ImGui::Image(tex_id, ImVec2(128, 128), uv0, uv1,
                     ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 128));
        ImGui::EndTooltip();
      }
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if (ImGui::ImageButton(tex_id, ImVec2(32, 32), ImVec2(0, 0),
    ...
    if (ImGui::TreeNode("Selectables")) {
      if (ImGui::TreeNode("Basic")) {
        ...
        if (ImGui::Selectable("5. I am double clickable", selected[3],
                              ImGuiSelectableFlags_AllowDoubleClick))
          if (ImGui::IsMouseDoubleClicked(0)) selected[3] = !selected[3];
        ImGui::TreePop();
      }
      ...
      if (ImGui::TreeNode("Grid")) {
        static bool selected[16] = {true,  false, false, false, false, true,
    ...
    if (ImGui::TreeNode("Color/Picker Widgets")) {
      ...
      ShowHelpMarker(
          ...
          "slider/text inputs.\nWith the ImGuiColorEditFlags_NoLabel flag you "
          "can pass a non-empty label which will only be used for the tooltip "
          "and picker popup.");
      ...
      ImGui::ColorButton("MyColor##3b", *(ImVec4*)&color, misc_flags,
                         ImVec2(80, 80));
      ...
      ImGui::Combo("Inputs Mode", &inputs_mode,
    ...
    if (ImGui::TreeNode("Vertical Sliders")) {
      ...
      for (int nx = 0; nx < 4; nx++) {
        ...
        for (int ny = 0; ny < rows; ny++) {
          ...
          ImGui::VSliderFloat("##v", small_slider_size, &values2[nx], 0.0f,
                              1.0f, "");
          if (ImGui::IsItemActive() || ImGui::IsItemHovered())
  ...
  if (ImGui::CollapsingHeader("Layout")) {
    if (ImGui::TreeNode("Child regions")) {
      ...
      ImGui::SameLine();
      ImGui::PushItemWidth(100);
      goto_line |= ImGui::InputInt("##Line", &line, 0, 0,
                                   ImGuiInputTextFlags_EnterReturnsTrue);
      ImGui::PopItemWidth();
      ImGui::BeginChild(
          "Sub1", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.5f, 300),
          false, ImGuiWindowFlags_HorizontalScrollbar);
      for (int i = 0; i < 100; i++) {
        ImGui::Text("%04d: scrollable region", i);
        if (goto_line && line == i) ImGui::SetScrollHere();
      }
      if (goto_line && line >= 100) ImGui::SetScrollHere();
      ImGui::EndChild();
    ...
    if (ImGui::TreeNode("Basic Horizontal Layout")) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        ImGui::PopID();
        // if (ImGui::IsItemHovered()) ImGui::SetTooltip("ListBox %d hovered",
        // i);
      }
      ImGui::PopItemWidth();
    ...
    if (ImGui::TreeNode("Groups")) {
      ImGui::TextWrapped(
          "(Using ImGui::BeginGroup()/EndGroup() to layout items. BeginGroup() "
          "basically locks the horizontal position. EndGroup() bundles the "
          "whole group so that you can use functions such as IsItemHovered() "
          "on it.)");
      ImGui::BeginGroup();
      {
        ImGui::BeginGroup();
    ...
    if (ImGui::TreeNode("Text Baseline Alignment")) {
      ImGui::TextWrapped(
          "(This is testing the vertical alignment that occurs on text to keep "
          "it at the same baseline as widgets. Lines only composed of text or "
          "\"small\" widgets fit in less vertical spaces than lines with "
          "normal widgets)");
      ...
      ImGui::AlignFirstTextHeightToWidgets();  // If your line starts with text,
      ...
      ImGui::AlignFirstTextHeightToWidgets();  // Vertically align text node a
                                               ...
                                               // fit).
      bool node_open =
          ImGui::TreeNode("Node##2");  // Common mistake to avoid: if we want to
                                       // SameLine after TreeNode we need to do
                                       // it before we add child content.
      ImGui::SameLine(0.0f, spacing);
    ...
    if (ImGui::TreeNode("Scrolling")) {
      ...
      ImGui::SameLine(130);
      track |=
          ImGui::DragInt("##line", &track_line, 0.25f, 0, 99, "Line = %.0f");
      bool scroll_to = ImGui::Button("Scroll To Pos");
      ImGui::SameLine(130);
      scroll_to |= ImGui::DragInt("##pos_y", &scroll_to_px, 1.00f, 0, 9999,
                                  "Y = %.0f px");
      ImGui::PopItemWidth();
    ...
    if (ImGui::TreeNode("Horizontal Scrolling")) {
      ImGui::Bullet();
      ImGui::TextWrapped(
          "Horizontal scrolling for a window has to be enabled explicitly via "
          "the ImGuiWindowFlags_HorizontalScrollbar flag.");
      ImGui::Bullet();
      ImGui::TextWrapped(
      ...
      ImGui::BeginChild(
          "scrolling",
          ImVec2(0, ImGui::GetItemsLineHeightWithSpacing() * 7 + 30), true,
          ImGuiWindowFlags_HorizontalScrollbar);
      for (int line = 0; line < lines; line++) {
        // Display random stuff (for the sake of this trivial demo we are using
        // basic Button+SameLine. If you want to create your own time line for a
        // real application you may be better off manipulating the cursor
        // position yourself, aka using SetCursorPos/SetCursorScreenPos to
        // position the widgets yourself. You may also want to use the
      ...
      if (scroll_x_delta != 0.0f) {
        ImGui::BeginChild("scrolling");  // Demonstrate a trick: you can use
                                         // Begin to set yourself in the context
                                         // of another window (here we are
                                         // already out of your child window)
    ...
    if (ImGui::TreeNode("Clipping")) {
      ...
      ImGui::TextWrapped(
          "On a per-widget basis we are occasionally clipping text CPU-side if "
          "it won't fit in its frame. Otherwise we are doing coarser clipping "
          "+ passing a scissor rectangle to the renderer. The system is "
          "designed to try minimizing both execution and CPU/GPU rendering "
          "cost.");
  ...
  if (ImGui::CollapsingHeader("Popups & Modal windows")) {
    if (ImGui::TreeNode("Popups")) {
      ...
      if (ImGui::Button("Select..")) ImGui::OpenPopup("select");
      ImGui::SameLine();
      ...
      if (ImGui::Button("Toggle..")) ImGui::OpenPopup("toggle");
      ...
      if (ImGui::Button("Popup Menu..")) ImGui::OpenPopup("FilePopup");
      ...
      // the parent of this menu from the parent of the various popup menus
      // above. To do so we are encloding the items in a PushID()/PopID() block
      // to make them two different menusets. If we don't, opening any popup
      // above and hovering our menu here would open it. This is because once a
      // menu is active, we allow to switch to a sibling menu by just hovering
    ...
    if (ImGui::TreeNode("Context menus")) {
      ...
      sprintf(buf, "Button: %s###Button",
              name);  // ### operator override ID ignoring the preceeding label
      ImGui::Button(buf);
    ...
    if (ImGui::TreeNode("Modals")) {
      ...
      if (ImGui::BeginPopupModal("Stacked 1")) {
        ImGui::Text(
            ...
            "style.Colors[ImGuiCol_ModalWindowDarkening] for darkening.");
        static int item = 1;
  ...
  if (ImGui::CollapsingHeader("Columns")) {
    ...
    */
    ...
    if (ImGui::TreeNode("Horizontal Scrolling")) {
      ...
      ImGui::BeginChild("##scrollingregion", ImVec2(0, 120), false,
                        ImGuiWindowFlags_HorizontalScrollbar);
      ImGui::Columns(10);
      for (int i = 0; i < 20; i++)
        for (int j = 0; j < 10; j++) {
  ...
  if (ImGui::CollapsingHeader("Inputs & Focus")) {
    ...
    ImGui::SameLine();
    ShowHelpMarker(
        "Request ImGui to render a mouse cursor for you in software. Note that "
        "a mouse cursor rendered via regular GPU rendering will feel more "
    ...
    if (ImGui::TreeNode("Tabbing")) {
      ...
      // ImGui::PushAllowKeyboardFocus(bool)\nto disable tabbing through certain
      // widgets.");
    ...
    if (ImGui::TreeNode("Focus from code")) {
      ...
      ImGui::TextWrapped(
          "Cursor & selection are preserved when refocusing last used item in "
          "code.");
      ImGui::TreePop();
    }
    ...
    if (ImGui::TreeNode("Dragging")) {
      ...
      if (ImGui::IsItemActive()) {
        ...
        draw_list->AddLine(
            ...
            io.MousePos, ImColor(ImGui::GetStyle().Colors[ImGuiCol_Button]),
            4.0f);
        ...
        ImGui::SameLine();
        ImGui::Text(
            "Raw (%.1f, %.1f), WithLockThresold (%.1f, %.1f), MouseDelta "
            "(%.1f, %.1f)",
    ...
    if (ImGui::TreeNode("Mouse cursors")) {
      ...
      ShowHelpMarker(
          ...
          "(io.MouseDrawCursor) is set ImGui will draw the right cursor for "
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
  ...
  // and save to (else it compares to the default style)
  const ImGuiStyle default_style;  // Default style
  if (ImGui::Button("Revert Style")) style = ref ? *ref : default_style;

  if (ref) {
    ImGui::SameLine();
    if (ImGui::Button("Save Style")) *ref = style;
  }
  ...
  if (ImGui::TreeNode("Rendering")) {
    ...
    ImGui::DragFloat("Curve Tessellation Tolerance",
                     &style.CurveTessellationTol, 0.02f, 0.10f, FLT_MAX, NULL,
                     2.0f);
  ...
  if (ImGui::TreeNode("Settings")) {
    ImGui::SliderFloat2("WindowPadding", (float*)&style.WindowPadding, 0.0f,
                        20.0f, "%.0f");
    ImGui::SliderFloat("WindowRounding", &style.WindowRounding, 0.0f, 16.0f,
                       "%.0f");
    ImGui::SliderFloat("ChildWindowRounding", &style.ChildWindowRounding, 0.0f,
                       16.0f, "%.0f");
    ImGui::SliderFloat2("FramePadding", (float*)&style.FramePadding, 0.0f,
                        20.0f, "%.0f");
    ImGui::SliderFloat("FrameRounding", &style.FrameRounding, 0.0f, 16.0f,
                       "%.0f");
    ImGui::SliderFloat2("ItemSpacing", (float*)&style.ItemSpacing, 0.0f, 20.0f,
                        "%.0f");
    ...
    ImGui::SliderFloat("GrabRounding", &style.GrabRounding, 0.0f, 16.0f,
                       "%.0f");
    ImGui::Text("Alignment");
    ImGui::SliderFloat2("WindowTitleAlign", (float*)&style.WindowTitleAlign,
                        0.0f, 1.0f, "%.2f");
    ImGui::SliderFloat2("ButtonTextAlign", (float*)&style.ButtonTextAlign, 0.0f,
                        1.0f, "%.2f");
    ImGui::SameLine();
    ShowHelpMarker(
  ...
  if (ImGui::TreeNode("Colors")) {
    ...
    if (ImGui::Button("Copy Colors")) {
      ...
      for (int i = 0; i < ImGuiCol_COUNT; i++) {
        ...
        if (!output_only_modified ||
            memcmp(&col, (ref ? &ref->Colors[i] : &default_style.Colors[i]),
                   sizeof(ImVec4)) != 0)
          ImGui::LogText(
    ...
    for (int i = 0; i < ImGuiCol_COUNT; i++) {
      ...
      if (memcmp(&style.Colors[i],
                 (ref ? &ref->Colors[i] : &default_style.Colors[i]),
                 sizeof(ImVec4)) != 0) {
        ImGui::SameLine();
  ...
  bool fonts_opened =
      ImGui::TreeNode("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size);
  ImGui::SameLine();
  ShowHelpMarker(
      "Tip: Load fonts with io.Fonts->AddFontFromFileTTF()\nbefore calling "
      "io.Fonts->GetTex* functions.");
  if (fonts_opened) {
    ...
    for (int i = 0; i < atlas->Fonts.Size; i++) {
      ...
      if (font_details_opened) {
        ...
        ShowHelpMarker(
            ...
            "size at the time of building the atlas. You may oversample them "
        ...
        if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size)) {
          ...
          const ImFontGlyph* glyph_fallback =
              font->FallbackGlyph;  // Forcefully/dodgily make FindGlyph()
                                    // return NULL on fallback, which isn't the
                                    // default behavior.
          ...
          for (int base = 0; base < 0x10000; base += 256) {
            ...
            if (count > 0 &&
              ...
              for (int n = 0; n < 256; n++) {
                ImVec2 cell_p1(
                    base_pos.x + (n % 16) * (cell_size.x + cell_spacing),
                    base_pos.y + (n / 16) * (cell_size.y + cell_spacing));
                ...
                font->RenderChar(
                    draw_list, cell_size.x, cell_p1,
                    ImGui::GetColorU32(ImGuiCol_Text),
                    (ImWchar)(base + n));  // We use ImFont::RenderChar as a
                                           // shortcut because we don't have
                                           // UTF-8 conversion functions
                                           // available to generate a string.
              ...
              ImGui::Dummy(ImVec2((cell_size.x + cell_spacing) * 16,
    ...
    ImGui::DragFloat("this window scale", &window_scale, 0.005f, 0.3f, 2.0f,
                     "%.1f");  // scale only this window
    ImGui::DragFloat("global scale", &ImGui::GetIO().FontGlobalScale, 0.005f,
                     0.3f, 2.0f, "%.1f");  // scale everything
    ImGui::PopItemWidth();
...
static void ShowExampleAppConstrainedResize(bool* p_open) {
  ...
  if (type == 3)
    ImGui::SetNextWindowSizeConstraints(ImVec2(300, 0),
                                        ImVec2(400, FLT_MAX));  // Width 300-400
  if (type == 4)
    ImGui::SetNextWindowSizeConstraints(
  ...
  if (ImGui::Begin("Example: Constrained Resize", p_open)) {
    const char* desc[] = {
        ...
        "Width > 100, Height > 100", "Width 300-400",
...
// Read section "How can I have multiple widgets with the same label? Can I have
// widget without a label? (Yes). A primer on the purpose of labels/IDs." about
// ID.
static void ShowExampleAppManipulatingWindowTitle(bool*) {
  ...
  sprintf(buf, "Animated title %c %d###AnimatedTitle",
          "|/-\\"[(int)(ImGui::GetTime() / 0.25f) & 3], rand());
...
static void ShowExampleAppCustomRendering(bool* p_open) {
  ...
  ImGui::ColorEdit3("Color", &col.x);
  {
    ...
    for (int n = 0; n < 2; n++) {
      ...
      draw_list->AddCircle(ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f,
                           col32, 20, thickness);
      x += sz + spacing;
      draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 0.0f, ~0,
                         thickness);
      x += sz + spacing;
      draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f, ~0,
                         thickness);
      x += sz + spacing;
      draw_list->AddTriangle(ImVec2(x + sz * 0.5f, y),
                             ImVec2(x + sz, y + sz - 0.5f),
                             ImVec2(x, y + sz - 0.5f), col32, thickness);
      x += sz + spacing;
      ...
      draw_list->AddBezierCurve(ImVec2(x, y),
                                ...
                                ImVec2(x + sz, y + sz), col32, thickness);
      x = p.x + 4;
      y += sz + spacing;
    }
    ...
    draw_list->AddRectFilledMultiColor(
        ImVec2(x, y), ImVec2(x + sz, y + sz), ImColor(0, 0, 0),
  ...
  ImGui::Separator();
  {
    ...
    if (adding_line) {
      ...
      if (!ImGui::GetIO().MouseDown[0]) adding_line = adding_preview = false;
    }
    if (ImGui::IsItemHovered()) {
    ...
    draw_list->PushClipRect(
        canvas_pos, ImVec2(canvas_pos.x + canvas_size.x,
                           canvas_pos.y + canvas_size.y));  // clip lines within
                                                            // the canvas (if we
                                                            // resize it, etc.)
...
struct ExampleAppConsole {
  ...
  ExampleAppConsole() {
    ...
    Commands.push_back("CLASSIFY");  // "classify" is here to provide an example
  ...
  void Draw(const char* title, bool* p_open) {
    ...
    // wanting coarse clipping and filtering may want to pre-compute an array of
    // indices that passed the filtering test, recomputing this array when user
    ...
    // example code! If your items are of variable size you may want to
    // implement code similar to what ImGuiListClipper does. Or split your data
    // into fixed height items to allow random-seeking into your list.
    ...
    for (int i = 0; i < Items.Size; i++) {
      ...
      ImVec4 col =
          ImVec4(1.0f, 1.0f, 1.0f, 1.0f);  // A better implementation may store
  ...
  static int TextEditCallbackStub(
      ImGuiTextEditCallbackData* data)  // In C++11 you are better off using
                                        // lambdas for this sort of forwarding
                                        // callbacks
  {
...
static void ShowExampleAppLayout(bool* p_open) {
  ...
  if (ImGui::Begin("Example: Layout", p_open, ImGuiWindowFlags_MenuBar)) {
    ...
    ImGui::BeginChild(
        "item view",
        ImVec2(0, -ImGui::GetItemsLineHeightWithSpacing()));  // Leave room for
                                                              // 1 line below us
...
static void ShowExampleAppLongText(bool* p_open) {
  ...
  ImGui::Combo("Test type", &test_type,
               ...
               "clipped manually\0Multiple calls to Text(), not clipped\0");
