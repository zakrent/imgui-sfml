...
//           for improved quality on small fonts stbtt_PackFontRanges()
//           -- pack and renders stbtt_PackEnd() stbtt_GetPackedQuad()
//
...
static stbtt__active_edge *stbtt__new_active(stbtt__hheap *hh, stbtt__edge *e,
  ...
  z->x = STBTT_ifloor(STBTT_FIX * e->x0 +
                      z->dx * (start_point - e->y0));  // use z->dx so when we
                                                       // offset later it's by
                                                       // the same amount
  z->x -= off_x * STBTT_FIX;
...
static int stbtt__tesselate_curve(stbtt__point *points, int *num_points,
  ...
  if (n > 16)  // 65536 segments on one curve better be enough!
    return 1;
  if (dx * dx + dy * dy > objspace_flatness_squared) {  // half-pixel error
                                                        // allowed... need to be
                                                        // smaller if AA
