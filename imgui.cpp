...
// company, please consider financial support, e.g:
...
/*
 ...
 content - creating a window / opening a tree node for the first time, etc. -
 ...
 - See examples/ folder for standalone sample applications. To understand the
 integration process, you can read examples/opengl2_example/ because it is
 short, then switch to the one more appropriate to your use case.
 - You may be able to grab and copy a ready made imgui_impl_*** file from the
 examples/.
 - When using Dear ImGui, your programming IDE if your friend: follow the
 ...
 render data. it will call your io.RenderDrawListFn handler. (if you don't need
 to render, you still need to call Render() and ignore the callback, or call
 EndFrame() instead. if you call neither some aspects of windows focusing/moving
 will appear broken.)
 ...
 set to NULL and call GetDrawData() after Render() to access the render data.
 ...
 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 MyEngineScissor((int)pcmd->ClipRect.x, (int)pcmd->ClipRect.y,
 (int)(pcmd->ClipRect.z - pcmd->ClipRect.x), (int)(pcmd->ClipRect.w -
 pcmd->ClipRect.y)); MyEngineDrawIndexedTriangles(pcmd->ElemCount,
 ...
 disappearing when I move windows around.. A: Most likely you are mishandling
 the clipping rectangles in your render function. Rectangles provided by ImGui
 are defined as (x1=left,y1=top,x2=right,y2=bottom) and NOT as
 (x1,y1,width,height).
 ...
 over time (1-1 relationship), using a static string as ID will preserve your
 node open/closed state when the targeted object change. e.g. when displaying a
 list of objects, using indices or pointers as ID will preserve the node
 open/closed state differently. experiment and see what makes more sense!
 ...
 on-screen keyboard, if available (e.g. on a mobile phone, or console without a
 keyboard). Preferably read the flags after calling ImGui::NewFrame() to avoid
 them lagging by one frame. But reading those flags before calling NewFrame() is
 also generally ok, as the bool toggles fairly rarely and you don't generally
 expect to interact with either Dear ImGui or your application during the same
 frame when that transition occurs. Dear ImGui is tracking dragging and widget
 ...
 have 'io.WantCaptureKeyboard=false'. Depending on your application logic it may
 or not be inconvenient. You might want to track which key-downs were for Dear
 ...
 'extra_fonts/README.txt' for instructions.
 ...
 code only once a frame. You can use it to quickly add custom UI in the middle
...
#pragma clang diagnostic ignored \
    "-Wunknown-pragmas"  // warning : unknown warning group '-Wformat-pedantic
                         // *'        // not all warnings are known by all clang
                         // versions.. so ignoring warnings triggers new
...
#pragma clang diagnostic ignored \
    "-Wexit-time-destructors"  // warning : declaration requires an exit-time
                               // destructor       // exit-time destruction
                               // order is undefined. if MemFree() leads to
                               // users code that has been disabled before exit
                               // it might cause problems. ImGui coding style
...
#pragma clang diagnostic ignored "-Wsign-conversion"  // warning : implicit
                                                      // conversion changes
                                                      // signedness //
#pragma clang diagnostic ignored \
...
#pragma GCC diagnostic ignored "-Wconversion"  // warning: conversion to 'xxxx'
                                               // from 'xxxx' may alter its
                                               // value
...
ImGuiStyle::ImGuiStyle() {
  ...
  ChildWindowRounding = 0.0f;  // Radius of child window corners rounding. Set
...
// trying to deduct it at compile time sounds like a pandora can of worm.
...
ImU32 ImHash(const void* data, int data_size, ImU32 seed) {
  ...
  if (data_size > 0) {
    ...
    while (unsigned char c = *current++) {
      ...
      // the hash, and only "label" gets displayed. Because this syntax is
      // rarely used we are optimizing for the common case.
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height) {
  ...
  window->DC.PrevLineHeight =
      (line_height - GImGui->Style.ItemSpacing.y);  // If we end up needing more
                                                    // accurate data (to e.g.
                                                    // use SameLine) we may as
                                                    // well make the clipper
                                                    // have a fourth step to let
                                                    // user process and display
                                                    // the last item in their
                                                    // list.
...
bool ImGui::IsItemHovered() {
  ...
  if (g.ActiveId != 0 && g.ActiveId != window->DC.LastItemId &&
    ...
    return false;
  if (!IsMouseHoveringRect(window->DC.LastItemRect.Min,
                           window->DC.LastItemRect.Max))
    return false;
  if (!IsWindowContentHoverable(window)) return false;
  return true;
}

bool ImGui::IsItemRectHovered() {
  ImGuiWindow* window = GetCurrentWindowRead();
  return IsMouseHoveringRect(window->DC.LastItemRect.Min,
                             window->DC.LastItemRect.Max);
}
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id,
  ...
  const bool allow_keyboard_focus =
      (window->DC.ItemFlags & ImGuiItemFlags_AllowKeyboardFocus) != 0;
    ...
    window->FocusIdxTabRequestNext =
        ...
        (g.IO.KeyShift ? (allow_keyboard_focus ? -1 : 0)
                       : +1);  // Modulo on index will be applied at the end of
                               // frame once we've got the total counter of
                               // items.
...
void ImGui::NewFrame() {
  ...
  IM_ASSERT(g.Style.Alpha >= 0.0f &&
            g.Style.Alpha <= 1.0f);  // Invalid style setting. Alpha cannot be
                                     // negative (allows us to avoid a few
                                     // clamps in color computations)

  // Initialize on first frame
  if (!g.Initialized) ImGui::Initialize();
  ...
  // component, but in reality we test for -256000.0f) we cancel out movement in
  // MouseDelta
  ...
  // avoid input lag or sheering). Only valid for root windows.
  ...
  // from the rest of their application. When clicking outside of a window we
  // assume the click is owned by the application and won't request capture. We
  // need to track click ownership.
  ...
  g.IO.WantCaptureKeyboard = (g.WantCaptureKeyboardNextFrame != -1)
  ...
  if (g.HoveredWindow && g.IO.MouseWheel != 0.0f &&
    ...
    if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling) {
      ...
      SetWindowScrollY(window, window->Scroll.y - g.IO.MouseWheel *
                                                      window->CalcFontSize() *
                                                      scroll_lines);
    }
  }
...
static void LoadIniSettingsFromDisk(const char* ini_filename) {
  ...
  int file_size;
  char* file_data =
      (char*)ImFileLoadToMemory(ini_filename, "rb", &file_size, 1);
  if (!file_data) return;
  ...
  for (const char* line_start = file_data; line_start < buf_end;) {
    ...
    if (line_start[0] == '[' && line_end > line_start && line_end[-1] == ']') {
      char name[64];
      ImFormatString(name, IM_ARRAYSIZE(name), "%.*s",
                     (int)(line_end - line_start - 2), line_start + 1);
      settings = FindWindowSettings(name);
      if (!settings) settings = AddWindowSettings(name);
    } else if (settings) {
      float x, y;
      int i;
...
static void SaveIniSettingsToDisk(const char* ini_filename) {
  ...
  for (int i = 0; i != g.Windows.Size; i++) {
    ...
    if (!settings)  // This will only return NULL in the rare instance where the
                    // window was first created with
                    // ImGuiWindowFlags_NoSavedSettings then had the flag
                    // disabled later on. We don't bind settings in this case
                    // (bug #1000).
      continue;
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list,
  ...
  // draw call (default ImDrawIdx = unsigned short = 2 bytes = 64K vertices per
  // window) If this assert triggers because you are drawing lots of stuff
  // manually, you can: A) Add '#define ImDrawIdx unsigned int' in imconfig.h to
  // set the index size to 4 bytes. You'll need to handle the 4-bytes indices to
  // your renderer.
  //    For example, the OpenGL example code detect index size at compile-time
  //    by doing: 'glDrawElements(GL_TRIANGLES, (GLsizei)pcmd->ElemCount,
  //    sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT,
  //    idx_buffer_offset);' Your own engine or render API may use different
  //    parameters or function calls to specify index sizes. 2 and 4 bytes
  //    indices are generally supported by most API.
  // B) If for some reason you cannot use 4 bytes indices or don't want to, a
  // workaround is to call BeginChild()/EndChild() before reaching the 64K limit
  // to split your draw commands in multiple draw lists.
  IM_ASSERT(((ImU64)draw_list->_VtxCurrentIdx >> (sizeof(ImDrawIdx) * 8)) ==
            0);  // Too many vertices in same ImDrawList. See comment above.
...
static void AddWindowToRenderListSelectLayer(ImGuiWindow* window) {
  // FIXME: Generalize this with a proper layering system so e.g. user can draw
  // in specific layers, below text, ..
  ImGuiContext& g = *GImGui;
...
void ImGui::EndFrame() {
  ...
  if (g.ActiveId == 0 && g.HoveredId == 0 && g.IO.MouseClicked[0]) {
    if (!(g.NavWindow && !g.NavWindow->WasActive &&
          g.NavWindow->Active))  // Unless we just made a popup appear
    {
...
bool ImGui::IsMouseHoveringRect(const ImVec2& r_min, const ImVec2& r_max,
  ...
  if (clip) rect_clipped.ClipWith(window->ClipRect);
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold) {
  ...
  if (g.IO.MouseDown[button])
    if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
      return g.IO.MousePos - g.IO.MouseClickedPos[button];  // Assume we can
                                                            // only get active
                                                            // with left-mouse
                                                            // button (at the
                                                            // moment).
...
static void BeginTooltipEx(ImGuiWindowFlags extra_flags,
                           bool override_previous_tooltip) {
  ImGuiContext& g = *GImGui;
  char window_name[16];
  ImFormatString(window_name, IM_ARRAYSIZE(window_name), "##Tooltip%02d",
                 g.TooltipOverrideCount);
  if (override_previous_tooltip)
    if (ImGuiWindow* window = ImGui::FindWindowByName(window_name))
      if (window->Active) {
        // Hide previous tooltips. We can't easily "reset" the content of a
...
void ImGui::BeginTooltip() { BeginTooltipEx(0, false); }
...
void ImGui::OpenPopupEx(ImGuiID id, bool reopen_existing) {
  ...
  ImGuiPopupRef popup_ref = ImGuiPopupRef(
      id, window, window->GetID("##menus"),
      g.IO.MousePos);  // Tagged as new ref because constructor sets Window to
                       // NULL (we are passing the ParentWindow info here)
  ...
  if (g.NavWindow) {
    for (n = 0; n < g.OpenPopupStack.Size; n++) {
      ...
      for (int m = n; m < g.OpenPopupStack.Size && !has_focus; m++)
        has_focus =
            (g.OpenPopupStack[m].Window &&
             g.OpenPopupStack[m].Window->RootWindow == g.NavWindow->RootWindow);
      if (!has_focus) break;
    }
  }
...
void ImGui::EndPopup() {
  ...
  End();
  if (!(window->Flags & ImGuiWindowFlags_Modal)) PopStyleVar();
}
...
//    that the item can be interacted with (because it is blocked by the active
//    popup) may useful in some situation when e.g. large canvas as one item,
//    content of menu driven by click position.
...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg,
  ...
  const int auto_fit_axises =
      ((size.x == 0.0f) ? 0x01 : 0x00) | ((size.y == 0.0f) ? 0x02 : 0x00);
  if (size.x <= 0.0f)
  ...
  char title[256];
  if (name)
    ImFormatString(title, IM_ARRAYSIZE(title), "%s.%s.%08X",
                   parent_window->Name, name, id);
  else
    ImFormatString(title, IM_ARRAYSIZE(title), "%s.%08X", parent_window->Name,
                   id);
...
void ImGui::EndChild() {
  ...
  if ((window->Flags & ImGuiWindowFlags_ComboBox) || window->BeginCount > 1) {
    ...
    if (window->AutoFitChildAxises & 0x01)  // Arbitrary minimum zero-ish child
                                            // size of 4.0f causes less trouble
                                            // than a 0.0f
      sz.x = ImMax(4.0f, sz.x);
    if (window->AutoFitChildAxises & 0x02) sz.y = ImMax(4.0f, sz.y);
    ImGui::End();
...
static ImVec2 FindBestPopupWindowPos(const ImVec2& base_pos, const ImVec2& size,
       ...
       n++)  // Last, Right, down, up, left. (Favor last used direction).
  {
    ...
    *last_dir = dir;
    return ImVec2(
        dir == 0 ? r_inner.Max.x
                 : dir == 3 ? r_inner.Min.x - size.x : base_pos_clamped.x,
        dir == 1 ? r_inner.Max.y
                 : dir == 2 ? r_inner.Min.y - size.y : base_pos_clamped.y);
  }
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size,
  ...
  if (flags & ImGuiWindowFlags_NoSavedSettings) {
    ...
    if (ImLengthSqr(settings->Size) > 0.00001f) size = settings->Size;
    window->Size = window->SizeFull = size;
  }
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window) {
  ...
  if ((flags & ImGuiWindowFlags_Tooltip) != 0) {
    ...
    // Handling case of auto fit window not fitting on the screen (on either
    // axis): we are growing the size on the other axis to compensate for
    // expected scrollbar. FIXME: Might turn bigger than
    ...
    ImVec2 size_auto_fit_after_constraint =
        CalcSizeFullWithConstraint(window, size_auto_fit);
    if (size_auto_fit_after_constraint.x < window->SizeContents.x &&
        !(flags & ImGuiWindowFlags_NoScrollbar) &&
        (flags & ImGuiWindowFlags_HorizontalScrollbar))
      size_auto_fit.y += style.ScrollbarSize;
    if (size_auto_fit_after_constraint.y < window->SizeContents.y &&
        !(flags & ImGuiWindowFlags_NoScrollbar))
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow* window) {
  ...
  if (window->ScrollTarget.y < FLT_MAX)
    scroll.y =
        ...
        (1.0f - cr_y) * (window->TitleBarHeight() + window->MenuBarHeight()) -
        cr_y * (window->SizeFull.y - window->ScrollbarSizes.y);
  scroll = ImMax(scroll, ImVec2(0.0f, 0.0f));
  if (!window->Collapsed && !window->SkipItems) {
    scroll.x =
        ImMin(scroll.x,
              ImMax(0.0f, window->SizeContents.x -
                              (window->SizeFull.x -
                               window->ScrollbarSizes
                                   .x)));  // == GetScrollMaxX for that window
    scroll.y =
        ImMin(scroll.y,
              ImMax(0.0f, window->SizeContents.y -
                              (window->SizeFull.y -
                               window->ScrollbarSizes
                                   .y)));  // == GetScrollMaxY for that window
  }
  return scroll;
}
...
// SetNextWindowSize(size, ImGuiCond_FirstUseEver) prior to calling Begin().
...
bool ImGui::Begin(const char* name, bool* p_open,
  ...
  if (flags & ImGuiWindowFlags_Popup) {
    ...
    window_just_activated_by_user |=
        (window->PopupId != popup_ref.PopupId);  // We recycle popups so treat
                                                 // window as activated if popup
                                                 // id changed
  ...
  if (g.SetNextWindowPosCond) {
    ...
    if (window_pos_set_by_api &&
        ImLengthSqr(g.SetNextWindowPosPivot) > 0.00001f) {
  ...
  for (root_non_popup_idx = root_idx; root_non_popup_idx > 0;
    ...
    if (!(g.CurrentWindowStack[root_non_popup_idx]->Flags &
        ...
        (g.CurrentWindowStack[root_non_popup_idx]->Flags &
         ImGuiWindowFlags_Modal))
      break;
  window->ParentWindow = parent_window;
  window->RootWindow = g.CurrentWindowStack[root_idx];
  window->RootNonPopupWindow =
      g.CurrentWindowStack[root_non_popup_idx];  // Used to display
                                                 // TitleBgActive color and for
                                                 // selecting which window to
                                                 // use for NavWindowing
  ...
  if (first_begin_of_the_frame) {
    ...
    if (window->Collapsed) {
      ...
      // width, But otherwise we don't honor ImGuiWindowFlags_AlwaysAutoResize
      // when collapsed.
    ...
    if ((flags & ImGuiWindowFlags_ChildWindow) &&
      ...
      window->Size = window->SizeFull =
          size_on_first_use;  // NB: argument name 'size_on_first_use'
                              // misleading here, it's really just 'size' as
                              // provided by user passed via
                              // BeginChild()->Begin().
    }
    ...
    if (window_pos_with_pivot) {
      ...
      // menu outside the parent bounds. This is how we end up with child menus
    ...
    if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api) {
      ...
      ImRect rect_to_avoid(ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 24,
                           ref_pos.y + 24);  // FIXME: Completely hard-coded.
                                             // Perhaps center on cursor
                                             // hit-point instead?
      window->PosFloat = FindBestPopupWindowPos(
          ref_pos, window->Size, &window->AutoPosLastDirection, rect_to_avoid);
      if (window->AutoPosLastDirection == -1)
        window->PosFloat =
            ref_pos + ImVec2(2, 2);  // If there's not enough room, for tooltip
                                     // we prefer avoiding the cursor at all
                                     // cost even if it means that part of the
    ...
    if (window->Collapsed) {
      ...
      if (window->ScrollbarX && !window->ScrollbarY)
        window->ScrollbarY =
            (window->SizeContents.y >
             window->Size.y + style.ItemSpacing.y - style.ScrollbarSize) &&
            !(flags & ImGuiWindowFlags_NoScrollbar);
      window->ScrollbarSizes =
      ...
      ImVec4 bg_color = style.Colors[bg_color_idx];  // We don't use
                                                     // GetColorU32() because
                                                     // bg_alpha is assigned
      ...
      if (bg_color.w > 0.0f)
        window->DrawList->AddRectFilled(
            ...
            (flags & ImGuiWindowFlags_NoTitleBar)
                ? ImGuiCorner_All
                : ImGuiCorner_BotLeft | ImGuiCorner_BotRight);

      // Title bar
      const bool is_focused =
          g.NavWindow &&
          window->RootNonPopupWindow == g.NavWindow->RootNonPopupWindow;
      if (!(flags & ImGuiWindowFlags_NoTitleBar))
        window->DrawList->AddRectFilled(
            title_bar_rect.GetTL(), title_bar_rect.GetBR(),
            GetColorU32(is_focused ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg),
  ...
  // doesn't lag by a frame Note that if our window is collapsed we will end up
  // with a null clipping rectangle which is the correct behavior.
  const ImRect title_bar_rect = window->TitleBarRect();
  const float border_size = window->BorderSize;
  // Force round to ensure that e.g. (int)(max.x-min.x) in user's render code
  // produce correct result.
  ImRect clip_rect;
  clip_rect.Min.x =
  ...
  if (flags & ImGuiWindowFlags_ChildWindow) {
    ...
    // border to the command list. (we could perform the check earlier in the
    // function but it is simpler at this point)
...
static void Scrollbar(ImGuiWindow* window, bool horizontal) {
  ...
  if (horizontal)
    window_rounding_corners =
        ImGuiCorner_BotLeft | (other_scrollbar ? 0 : ImGuiCorner_BotRight);
  else
    window_rounding_corners = (((window->Flags & ImGuiWindowFlags_NoTitleBar) &&
                                !(window->Flags & ImGuiWindowFlags_MenuBar))
                                   ...
                                   : 0) |
                              (other_scrollbar ? 0 : ImGuiCorner_BotRight);
...
void ImGui::TextUnformatted(const char* text, const char* text_end) {
  ...
  if (text_end - text > 2000 && !wrap_enabled) {
    ...
    // Optimization only available when word-wrapping is disabled. We also don't
    // vertically center the text within the line full height, which is unlikely
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered,
  ...
  if (hovered) {
    if (!(flags & ImGuiButtonFlags_NoKeyModifiers) ||
      ...
      // over bounds PressedOnClick         |  <on click>     |  <on click> <on
      // repeat> <on repeat> .. PressedOnRelease       |  <on release>   |  <on
      // repeat> <on repeat> .. (NOT on release) PressedOnDoubleClick   |  <on
      // dclick>    |  <on dclick> <on repeat> <on repeat> ..
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  // ItemSpacing (Ideally we'd want to add a flag for the user to specify we
  // want want the hit test to be done up to the right side of the content or
  // not)
...
static bool DataTypeApplyOpFromText(const char* buf,
  ...
  if (data_type == ImGuiDataType_Int) {
    ...
    float arg1f = 0.0f;
    if (sscanf(buf, scalar_format, &arg1f) < 1) return false;
    if (op == '+') {
      *v = arg0f + arg1f;
    }  // Add (use "+-" to subtract)
    else if (op == '*') {
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect& aabb,
  ...
  // allow for a TAB focus to happen) On the first frame, g.ScalarAsInputTextId
  // == 0, then on subsequent frames it becomes == id
  ...
  {
    IM_ASSERT(g.ActiveId == id);  // InputText ID expected to match the Slider
                                  // ID (else we'd need to store them both,
...
float ImGui::RoundScalar(float value, int decimal_precision) {
  ...
  if (decimal_precision < 0) return value;
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* text_begin,
  ...
  if (out_offset)
    *out_offset = ImVec2(line_width,
                         text_size.y + line_height);  // offset allow for the
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size,
  ...
  if (focus_requested || user_clicked || user_scrolled) {
    if (g.ActiveId != id) {
      ...
      edit_state.InitialText.resize(buf_size + 1);  // UTF-8. we use +1 to make
                                                    // sure that .Data isn't
                                                    // NULL so it doesn't crash.
      ...
      edit_state.CurLenA = (int)(buf_end - buf);  // We can't get the result
                                                  // from ImFormatString() above
                                                  // because it is not UTF-8
                                                  // aware. Here we'll cut off
                                                  // malformed UTF-8.
  ...
  if (g.ActiveId == id) {
    ...
    if (io.InputCharacters[0]) {
      ...
      // will effectively send a Return?) We ignore CTRL inputs, but need to
      // allow CTRL+ALT as some keyboards (e.g. German) use AltGR - which is
  ...
  if (g.ActiveId == id || is_currently_scrolling) {
    ...
    if (edit_state.StbState.select_start != edit_state.StbState.select_end) {
      ...
      float bg_offy_up =
          is_multiline ? 0.0f : -1.0f;  // FIXME: those offsets should be part
                                        // of the style? they don't play so well
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast,
  ...
  if (decimal_precision < 0)
    strcpy(display_format, "%f");  // Ideally we'd have a minimum decimal
                                   ...
                                   // non-significant digits? %f doesn't have a
                                   // minimum of 1
  else
...
bool ImGui::BeginCombo(const char* label, const char* preview_value,
  ...
  RenderCollapseTriangle(
      ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y,
             frame_bb.Min.y + style.FramePadding.y),
      true);
  ...
  if (popup_size.x == 0.0f) popup_size.x = w;

  float popup_y1 = frame_bb.Max.y;
  float popup_y2 = ImClamp(popup_y1 + popup_size.y, popup_y1,
                           g.IO.DisplaySize.y - style.DisplaySafeAreaPadding.y);
  if ((popup_y2 - popup_y1) <
      ImMin(popup_size.y, frame_bb.Min.y - style.DisplaySafeAreaPadding.y)) {
    // Position our combo ABOVE because there's more space to fit! (FIXME:
    // Handle in Begin() or use a shared helper. We have similar code in Begin()
    // for popup placement)
    popup_y1 = ImClamp(frame_bb.Min.y - popup_size.y,
    ...
    SetNextWindowPos(ImVec2(frame_bb.Min.x, frame_bb.Max.y), ImGuiCond_Always,
                     ImVec2(0.0f, 0.0f));
  }
  SetNextWindowSize(ImVec2(popup_size.x, popup_y2 - popup_y1),
                    ImGuiCond_Appearing);
  PushStyleVar(ImGuiStyleVar_WindowPadding, style.FramePadding);
...
bool ImGui::Combo(const char* label, int* current_item,
  ...
  for (int i = 0; i < items_count; i++) {
    ...
    if (item_selected && IsWindowAppearing()) SetScrollHere();
    PopID();
  }
...
bool ImGui::Selectable(const char* label, bool selected,
  ...
  if ((flags & ImGuiSelectableFlags_SpanAllColumns) &&
      window->DC.ColumnsCount > 1)  // FIXME-OPT: Avoid if vertically clipped.
    PopClipRect();
...
bool ImGui::ListBoxHeader(const char* label, int items_count,
                          int height_in_items) {
  // Size default to hold ~7 items. Fractional number of items helps seeing that
  // we can scroll down/up without looking at scrollbar. However we don't add
  // +0.40f if items_count <= height_in_items. It is slightly dodgy, because it
  // means a dynamic list of items will make the widget resize occasionally when
  // it crosses that size. I am expecting that someone will come and complain
  // about this behavior in a remote future, then we can advise on a better
  // solution.
...
bool ImGui::ListBox(const char* label, int* current_item,
  ...
  ImGuiListClipper clipper(
      items_count, GetTextLineHeightWithSpacing());  // We know exactly our line
                                                     // height here so we pass
                                                     // it as a minor
                                                     // optimization, but
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
  ...
  if (menuset_is_open)
    g.NavWindow = window;  // Odd hack to allow hovering across menus of a same
                           // menu-set (otherwise we wouldn't be able to hover
                           // parent)
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    window->DC.CursorPos.x +=
        (float)(int)(style.ItemSpacing.x *
                     (-1.0f + 0.5f));  // -1 spacing to compensate the spacing
                                       // added when Selectable() did a
                                       // SameLine(). It would also work to call
                                       // SameLine() ourselves after the
                                       // PopStyleVar().
  } else {
    // Menu inside a menu
  ...
  if (window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) {
    ...
    want_open = (!menu_is_open && hovered && !moving_within_opened_triangle) ||
             ...
             menuset_is_open)  // Menu bar: click an open menu again to close it
  {
    want_close = true;
  ...
  if (menu_is_open) {
    ...
    menu_is_open = BeginPopupEx(id, flags);  // menu_is_open can be 'false' when
                                             // the popup is completely clipped
...
void ImGui::ColorTooltip(const char* text, const float* col,
  ...
  ColorButton(
      "##preview", ImVec4(col[0], col[1], col[2], col[3]),
    ...
    for (float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++) {
      ...
      for (float x = p_min.x + grid_off.x + (yi & 1) * grid_step; x < p_max.x;
        ...
        if (y1 <= p_min.y) {
          ...
          if (x2 >= p_max.x)
            rounding_corners_flags_cell |= ImGuiCorner_TopRight;
        }
        if (y2 >= p_max.y) {
          if (x1 <= p_min.x) rounding_corners_flags_cell |= ImGuiCorner_BotLeft;
          if (x2 >= p_max.x)
            rounding_corners_flags_cell |= ImGuiCorner_BotRight;
        }
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
  ...
  if ((flags & ImGuiColorEditFlags__InputsMask) == 0)
    flags |=
        ImGuiColorEditFlags__OptionsDefault & ImGuiColorEditFlags__InputsMask;
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col,
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f) {
    ...
    RenderColorRectWithAlphaCheckerboard(
        ImVec2(bb.Min.x + grid_step, bb.Min.y), bb.Max, GetColorU32(col),
        grid_step, ImVec2(-grid_step, 0.0f), rounding,
        ImGuiCorner_TopRight | ImGuiCorner_BotRight);
    window->DrawList->AddRectFilled(bb.Min, ImVec2(mid_x, bb.Max.y),
                                    GetColorU32(col_without_alpha), rounding,
                                    ImGuiCorner_TopLeft | ImGuiCorner_BotLeft);
  } else {
    RenderColorRectWithAlphaCheckerboard(
        bb.Min, bb.Max,
        GetColorU32((flags & ImGuiColorEditFlags_AlphaPreview)
                        ? col
                        : col_without_alpha),
        grid_step, ImVec2(0, 0), rounding);
  }
  if (window->Flags & ImGuiWindowFlags_ShowBorders)
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags) {
  ...
  if (allow_opt_inputs) {
    if (RadioButton("RGB", (opts & ImGuiColorEditFlags_RGB) ? 1 : 0))
      opts =
          (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_RGB;
    if (RadioButton("HSV", (opts & ImGuiColorEditFlags_HSV) ? 1 : 0))
      opts =
          (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_HSV;
    if (RadioButton("HEX", (opts & ImGuiColorEditFlags_HEX) ? 1 : 0))
      opts =
          (opts & ~ImGuiColorEditFlags__InputsMask) | ImGuiColorEditFlags_HEX;
  }
  if (allow_opt_datatype) {
    ...
    if (RadioButton("0.00..1.00", (opts & ImGuiColorEditFlags_Float) ? 1 : 0))
      opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) |
  ...
  if (BeginPopup("Copy")) {
    int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]),
        ...
        ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255
                                                   : IM_F32_TO_INT8_SAT(col[3]);
    char buf[64];
    sprintf(buf, "(%.3ff, %.3ff, %.3ff, %.3ff)", col[0], col[1], col[2],
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, float* ref_col) {
  ...
  if (allow_opt_picker) {
    ImVec2 picker_size(
        g.FontSize * 8,
        ImMax(g.FontSize * 8 - (SmallSquareSize() + g.Style.ItemInnerSpacing.x),
...
bool ImGui::ColorEdit4(const char* label, float col[4],
  ...
  if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 &&
    ...
    for (int n = 0; n < components; n++) {
      ...
      if (!(flags & ImGuiColorEditFlags_NoOptions) && IsItemHovered() &&
          IsMouseClicked(1))
        OpenPopup("context");
    }
    PopItemWidth();
...
static void PaintVertsLinearGradientKeepAlpha(ImDrawVert* vert_start,
                                              ImDrawVert* vert_end,
                                              ImVec2 gradient_p0,
                                              ImVec2 gradient_p1, ImU32 col0,
                                              ImU32 col1) {
...
bool ImGui::ColorPicker4(const char* label, float col[4],
  ...
  if (!(flags & ImGuiColorEditFlags_NoSidePreview)) {
    ...
    if (ref_col != NULL) {
      ...
      if (ColorButton("##original", ref_col_v4,
                      ...
                      ImVec2(square_sz * 3, square_sz * 2))) {
        memcpy(col, ref_col,
               ((flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4) * sizeof(float));
        value_changed = true;
      }
    }
    EndGroup();
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    for (int n = 0; n < 6; n++) {
      ...
      PaintVertsLinearGradientKeepAlpha(
          draw_list->_VtxWritePtr -
              (draw_list->_VtxCurrentIdx - vert_start_idx),
          draw_list->_VtxWritePtr, gradient_p0, gradient_p1, hue_colors[n],
          hue_colors[n + 1]);
    }
  ...
  if (alpha_bar) {
    ...
    draw_list->AddRectFilledMultiColor(
        bar1_bb.Min, bar1_bb.Max, col32_no_alpha, col32_no_alpha,
...
void ImGui::Separator() {
  ...
  if (flags & ImGuiSeparatorFlags_Horizontal) {
    ...
    const ImRect bb(
        window->DC.CursorPos,
        window->DC.CursorPos + ImVec2(1.0f, window->DC.CurrentLineHeight));
...
void ImGui::EndGroup() {
  ...
  // 'bool LastItemIsActive', but if you search for LastItemId you'll notice it
  // is only used in that context.
...
static float PixelsToOffsetNorm(ImGuiWindow* window, float offset) {
  return (offset - window->DC.ColumnsMinX) /
...
float ImGui::GetColumnOffset(int column_index) {
  ...
  */
  ...
  const float x_offset =
      ImLerp(window->DC.ColumnsMinX, window->DC.ColumnsMaxX, t);
  return x_offset;
}
...
void ImGui::SetColumnOffset(int column_index, float offset) {
  ...
  const bool preserve_width =
      !(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoPreserveWidths) &&
      (column_index < window->DC.ColumnsCount - 1);
  ...
  if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoForceWithinWindow))
    offset = ImMin(offset, window->DC.ColumnsMaxX -
                               g.Style.ColumnsMinSpacing *
                                   (window->DC.ColumnsCount - column_index));
...
float ImGui::GetColumnWidth(int column_index) {
  ...
  return OffsetNormToPixels(
      window, window->DC.ColumnsData[column_index + 1].OffsetNorm -
...
void ImGui::PushColumnClipRect(int column_index) {
  ...
  PushClipRect(window->DC.ColumnsData[column_index].ClipRect.Min,
               window->DC.ColumnsData[column_index].ClipRect.Max, false);
}

void ImGui::BeginColumns(const char* id, int columns_count,
                         ImGuiColumnsFlags flags) {
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = GetCurrentWindow();

  IM_ASSERT(columns_count > 1);
  IM_ASSERT(window->DC.ColumnsCount ==
            1);  // Nested columns are currently not supported
  ...
  // -((window->Flags & ImGuiWindowFlags_NoScrollbar) ? 0 :
  ...
  for (int column_index = 0; column_index < columns_count + 1; column_index++) {
    ...
    if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoForceWithinWindow))
      t = ImMin(t, PixelsToOffsetNorm(window, window->DC.ColumnsMaxX -
                                                  g.Style.ColumnsMinSpacing *
                                                      (window->DC.ColumnsCount -
                                                       column_index)));
  ...
  for (int column_index = 0; column_index < columns_count; column_index++) {
    float clip_x1 =
        ImFloor(0.5f + window->Pos.x + GetColumnOffset(column_index) - 1.0f);
    float clip_x2 = ImFloor(0.5f + window->Pos.x +
                            GetColumnOffset(column_index + 1) - 1.0f);
    window->DC.ColumnsData[column_index].ClipRect =
...
void ImGui::EndColumns() {
  ...
  window->DC.CursorMaxPos.x =
      ImMax(window->DC.ColumnsStartMaxPosX,
            window->DC.ColumnsMaxX);  // Columns don't grow parent

  // Draw columns borders and handle resize
  if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoBorder) &&
      !window->SkipItems) {
    ...
    for (int i = 1; i < window->DC.ColumnsCount; i++) {
      ...
      const ImRect column_rect(ImVec2(x - column_w, y1),
                               ImVec2(x + column_w, y2));
      ...
      if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoResize)) {
        ...
        if (held && g.ActiveIdIsJustActivated)
          g.ActiveIdClickOffset.x -=
              column_w;  // Store from center of column line (we used a 8 wide
                         // rect for columns clicking). This is used by
                         // GetDraggedColumnOffset().
        if (held) dragging_column = i;
      }
...
// [2017/08: This is currently the only public API, while we are working on
  ...
  if (ImGui::Begin("ImGui Metrics", p_open)) {
    ...
    struct Funcs {
      static void NodeDrawList(ImDrawList* draw_list, const char* label) {
        ...
        for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin();
          ...
          ImGuiListClipper clipper(pcmd->ElemCount /
                                   3);  // Manually coarse clip our print out of
                                        // individual vertices to save CPU, only
      ...
      static void NodeWindow(ImGuiWindow* window, const char* label) {
        ...
        ImGui::BulletText("Scroll: (%.2f,%.2f)", window->Scroll.x,
                          window->Scroll.y);
        ImGui::BulletText("Active: %d, Accessed: %d", window->Active,
                          window->Accessed);
    ...
    if (ImGui::TreeNode("Basic state")) {
      ...
      ImGui::Text("HoveredId: 0x%08X/0x%08X", g.HoveredId,
                  g.HoveredIdPreviousFrame);  // Data is "in-flight" so
