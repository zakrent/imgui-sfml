...
static sf::Texture* s_fontTexture = NULL;  // owning pointer to internal font
                                           // atlas which is used if user
...
void Update(sf::Window& window, sf::RenderTarget& target, sf::Time dt) {
  ...
  window.setMouseCursorVisible(
      !ImGui::GetIO()
           .MouseDrawCursor);  // don't draw mouse cursor if ImGui draws it
}
...
void Update(const sf::Vector2i& mousePos, const sf::Vector2f& displaySize,
  ...
  assert(
      io.Fonts->Fonts.Size >
      0);  // You forgot to create and set up font atlas (see createFontTexture)
  ImGui::NewFrame();
}
...
void Shutdown() {
  ...
  ImGui::Shutdown();  // need to specify namespace here, otherwise
                      // ImGui::SFML::Shutdown would be called
}
...
void Image(const sf::Texture& texture, const sf::Vector2f& size,
  ...
  ImGui::Image((void*)texture.getNativeHandle(), size, ImVec2(0, 0),
               ImVec2(1, 1), tintColor, borderColor);
}
...
void Image(const sf::Texture& texture, const sf::Vector2f& size,
  ...
  ImGui::Image((void*)texture.getNativeHandle(), size, uv0, uv1, tintColor,
               borderColor);
}
...
bool ImageButton(const sf::Sprite& sprite, const int framePadding,
  ...
  return ImageButton(sprite, sf::Vector2f(spriteSize.width, spriteSize.height),
                     framePadding, bgColor, tintColor);
}
...
bool ImageButton(const sf::Sprite& sprite, const sf::Vector2f& size,
  ...
  return ::imageButtonImpl(*texturePtr,
                           static_cast<sf::FloatRect>(sprite.getTextureRect()),
...
bool imageButtonImpl(const sf::Texture& texture,
  ...
  return ImGui::ImageButton((void*)texture.getNativeHandle(), size, uv0, uv1,
                            framePadding, bgColor, tintColor);
}
